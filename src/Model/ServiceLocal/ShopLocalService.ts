    ...
    export class ShopLocalService {
        ...
        public static setShopList() {
            if (ShopLocalService.shopList) {
                for (var i = 0; i < ShopLocalService.shopList.length; i++) {
                    ShopLocalService.shopList[i].silver = ShopLocalService.setShopGold(ShopLocalService.shopList[i].st);
                }
            } else {
                ...
                for (var i = 0; i < WebValue.dataStModel.shopList.length; i++) {
                    ...
                    data.silver = ShopLocalService.setShopGold(WebValue.dataStModel.shopList[i]);
       ...
       * @商城5000元宝购买金币=100元宝购钱*1000000
        ...
        public static commitShop(_data: ShopData, _isFree: boolean, _onCallBack?: Function) {
            ...
            var myDate = new Date();
            _data.dy = new ShopDyModel(_data.st.id, Math.round(myDate.getTime() / 1000), _data.silver);//这里获得的时间是ms，到最终用于计算的是s
        ...
        public static upGradeShop(_data: ShopData, _isFree: boolean, _onCallBack: Function) {
            ...
            switch (_data.st.id) {
                case 1:
                    ...
                    ShopLocalService.addMoney(_data.silver, 2);
                    break;
                case 2:
                    ShopLocalService.addMoney(_data.silver, 5);
                    break;
                case 3:
                    ShopLocalService.addMoney(_data.silver, 10);
                    break;
                case 4:
                    ShopLocalService.protectFriend();
                    break;
